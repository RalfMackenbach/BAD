...
for idx, lam_val in enumerate(lam_arr):
    f = 1 - lam_val * modb
    tau_b_arr               = dldtheta
    bounce_time, roots      = bounce_int.bounce_integral_wrapper_safe(f,tau_b_arr,theta,return_roots=True)
    alpha_arr               = dldtheta * ( lam_val * grad_alpha + 2 * ( 1/modb - lam_val ) * curv_alpha )
    num_arr_alpha           = bounce_int.bounce_integral_wrapper_safe(f,alpha_arr,theta)
    psi_arr                 = dldtheta * ( lam_val * grad_psi   + 2 * ( 1/modb - lam_val ) * curv_psi )
    num_arr_psi             = bounce_int.bounce_integral_wrapper_safe(f,psi_arr,theta)
...
ax021=ax[2].twinx()
ax021.scatter(np.asarray(roots_bounceplot)/np.pi,np.asarray(walpha_bounceplot) * a_minor * dydalpha ,s=0.2,marker='.',alpha=1.0)
ax[3].plot(theta/np.pi,Bref*modb,color='black')
ax031=ax[3].twinx()
ax031.scatter(np.asarray(roots_bounceplot)/np.pi,np.asarray(wpsi_bounceplot) * drdpsi * a_minor ,s=0.2,marker='.',alpha=1.0)
...
ax[2].set_ylabel(r'$B \quad \mathrm{[T]}$')
ax[3].set_ylabel(r'$B \quad \mathrm{[T]}$')
