...
for res_idx, res_val in enumerate(res):
    ...
    lam_arr     = np.linspace(1/np.max(modb_arr),1/np.min(modb_arr),lam_res,endpoint=False)
    lam_arr     = np.delete(lam_arr,  0)
    cquad_num  = np.empty_like(lam_arr)
    cquad_den  = np.empty_like(lam_arr)
    cquad_ave  = np.empty_like(lam_arr)
    # make interpolated functions
    kind='cubic'
    ...
    for idx, lam_val in enumerate(lam_arr):
        ...
        cquad_num[idx] = d_alpha[0]
        cquad_ave[idx] = d_alpha[0]/tau_b[0]
    tot_time = time.time() - start_time
    # calculate error
    true_res = drift(lam_arr)
    cquad_err[res_idx] = np.average(np.abs((cquad_ave-true_res)/true_res))
    cquad_time[res_idx] = tot_time/len(lam_arr)
